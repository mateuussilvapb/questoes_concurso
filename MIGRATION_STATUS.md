...
> Gerado em 2026-07-28, atualizado em 2026-08-07 — baseado exclusivamente na análise do código-fonte.

> **Nota de escopo — migração de `historico` (06/08):** este módulo foi migrado inteiramente na camada de infraestrutura/persistência (entity, repository, service, validator, `IntegrityService`).

> **Nota de escopo — wiring de histórico na UI de resolução (07/08):** `resolver-questoes-page.ts` agora persiste um `HistoricoEntity` por questão respondida ao final de cada sessão de resolução completa, via `HistoricoService.criar()`. Decisão tomada com o usuário: **persistência em lote ao final**, não por questão — consistente com o aviso já existente em `resolver-questoes-component.ts` ("Os resultados não serão persistidos no histórico" ao encerrar antes de terminar todas as questões). `tempoResposta` por questão passou a ser medido por um cronômetro local no `ResolverQuestoesComponent` (independente do `TimerService` global, que segue medindo o tempo total da sessão exibido nos resultados), reiniciado a cada troca de questão (`proximaQuestao`/`questaoAnterior`/`irParaQuestao`) e a cada edição de questão em andamento (`onQuestaoEditada`). Verificado ponta a ponta no navegador (5 questões respondidas, histórico gravado no IndexedDB com `tempoResposta` individual, `correta`, `dificuldade`, `idMateria`, `idsAssuntos`) em 07/08. `pages/estatisticas/` (consumo de leitura do histórico) ainda não foi implementada — fora de escopo até ser pedida explicitamente.
